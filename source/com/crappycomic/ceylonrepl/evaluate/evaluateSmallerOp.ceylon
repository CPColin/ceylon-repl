...
Boolean|SyntaxError evaluateSmallerOp(Context context, Tree.SmallerOp operation) {
    value leftTerm = operation.leftTerm;
    value rightTerm = operation.rightTerm;
    
    if (!exists leftTerm) {
        return SyntaxError("Cannot evaluate a SmallerOp with no left term");
    }
    
    if (!exists rightTerm) {
        return SyntaxError("Cannot evaluate a SmallerOp with no right term");
    }
