...
Boolean|SyntaxError evaluateNotEqualOp(Context context, Tree.NotEqualOp operation) {
    value leftTerm = operation.leftTerm;
    value rightTerm = operation.rightTerm;
    
    if (!exists leftTerm) {
        return SyntaxError("Cannot evaluate a NotEqualOp with no left term");
    }
    
    if (!exists rightTerm) {
        return SyntaxError("Cannot evaluate a NotEqualOp with no right term");
    }
