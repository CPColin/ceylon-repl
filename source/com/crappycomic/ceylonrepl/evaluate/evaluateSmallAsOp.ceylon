...
Boolean|SyntaxError evaluateSmallAsOp(Context context, Tree.SmallAsOp operation) {
    value leftTerm = operation.leftTerm;
    value rightTerm = operation.rightTerm;
    
    if (!exists leftTerm) {
        return SyntaxError("Cannot evaluate a SmallAsOp with no left term");
    }
    
    if (!exists rightTerm) {
        return SyntaxError("Cannot evaluate a SmallAsOp with no right term");
    }
